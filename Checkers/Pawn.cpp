...
{
	if (pieceColor == white) //ruch bialego pionka
	{
		if (target.rank - source.rank == 1 && abs(target.file - source.file) == 1) //jesli ruszamy sie do gory o jeden po skosie
		{
			if (motherboard->whatStandsThere(target.file, target.rank) == emptySquare) //jezeli jest tam pusto to zwracamy prawde, ruch jest mozliwy do wykonania
			{
	...
	else //(pieceColor == black) ruch pionka czarnego
	{
