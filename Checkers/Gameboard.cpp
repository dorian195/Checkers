...
Gameboard::Gameboard(int boardSize_, int rowOfPawns_) : boardSize{ boardSize_ }, rowOfPawns{ rowOfPawns_ }
{
	...
	board_temp.resize(boardSize); //inicjalizuje szachownice pomocnicza
...
bool Gameboard::checkForAnyPossibleMoves() //sprawdza, czy aktualny gracz ma jakikolwiek mozliwy ruch
{
	if (whoseMove == white)
	{
		for (Piece * piece : whitePieces) //iteruj po wszystkich bierkach gracza bialego
		{
	...
	{
		for (Piece * piece : blackPieces) //iteruj po wszystkich bierkach gracza czarnego
		{
			if (piece->canMove() == true) return true; //jesli jakis pionek moze sie ruszyc, przerwij petle i zwroc true
		}
	}
...
bool Gameboard::tryToParse(string move_line) //probuje parsowac linie gracza, przerobic ja na ruchy
{
	actualMove.clear();
	stringstream ss;
	ss << move_line;
	...
	while (ss >> single_move) //pobranie slowa ze stringa
	{
		...
		if (firstCord >= 0 && firstCord < boardSize)
		{
			...
			if (s >> secondCord) //proba przerobienia stringa na liczbe int
			{
				...
				{
					actualMove.push_back(Position{ firstCord, secondCord }); //w przypadku powodzenia, funkcja dodaje zczytana pozycje do wektora pozycji, ktory reprezentuje pojedynczy ruch
					continue;
				}
			}
		}
...
{
	copyRealToTempBoard();
	if (actualMove.size() < 2) return false; //ruch nie sklada sie z 2 znakow
	if (whatStandsThere(actualMove[0].file, actualMove[0].rank) == emptySquare) return false; //nie chwyta za bierke
	if (whoseMove == white)
	{
		if (whatStandsThere(actualMove[0].file, actualMove[0].rank) == blackPiece) return false;
		for (int i = 1; i < actualMove.size(); i++)
		{
			if (board_temp[actualMove[i-1].rank][actualMove[i-1].file].getPiece()->makeSingleStep(actualMove[i-1], actualMove[i]) == false) return false; //wywoluje funkcje makeSingleStep w celu sprawdzenia czy dana bierka moze wykonac dany ruch
		}
		copyTempToRealBoard();
		return true;
	}
	if (whoseMove == black)
	{
		...
		for (int i = 1; i < actualMove.size(); i++)
		{
			if (board_temp[actualMove[0].rank][actualMove[0].file].getPiece()->makeSingleStep(actualMove[i - 1], actualMove[i]) == false) return false; //wywoluje funkcje makeSingleStep w celu sprawdzenia czy dana bierka moze wykonac dany ruch
		}
		copyTempToRealBoard();
		return true;
	}
